...
//		CharAt(int) : 파라미터로 받은 인덱스의 문자(char) 리턴
		...
		// equals(Object) : 파라미터로 받은 객체 s6와 equals가 실행한 객체 s1의 문자열을 비교한다.
		//				같으면 true, 다르면 false
		String s4 = "spr";
		String s5 = "ing";
		String s6 = s4 + s5;
		...
		System.out.println(s7.indexOf("책을")); // 2 처음 발견된 위치
		System.out.println(s7.indexOf("을")); // 3 (스페이스도 포함해서 숫자 셈 0 1 2 3 )
		System.out.println(s7.indexOf("자바")); // 없으면 -1
		
		//substring(int, int) : (시작인덱스, 종료인덱스+1) // 이 문자열의 부분 문자를 리턴한다.
		System.out.println(s7.substring(8,11)); // 무엇을
		System.out.println(s7.substring(12,16)); // "배울 수"
		
	// substring(int) : 이 문자열의 부분 문자열을 리턴한다. 종료인덱스 없으니까 끝까지 리턴함.
